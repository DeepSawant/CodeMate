...
// Enhanced C course with comprehensive lessons
add('c', [
  { id: 'c4', title: 'Advanced Control Flow & Functions', body: '<h2>Mastering Control Flow and Functions in C</h2><p>Control flow statements and functions are fundamental building blocks for creating efficient C programs.</p><h3>Advanced Loop Techniques</h3><pre><code>// Nested loops for matrix operations<br>void printMatrix(int matrix[][3], int rows) {<br>    for (int i = 0; i < rows; i++) {<br>        for (int j = 0; j < 3; j++) {<br>            printf("%d ", matrix[i][j]);<br>        }<br>        printf("\\n");<br>    }<br>}<br><br>// Function pointers and callbacks<br>typedef int (*operation)(int, int);<br>int add(int a, int b) { return a + b; }<br>int multiply(int a, int b) { return a * b; }<br><br>operation ops[] = {add, multiply};<br><br>int main() {<br>    int result = ops[0](5, 3);<br>    printf("Result: %d\\n", result);<br>    return 0;<br>}</code></pre>' },
  { id: 'c5', title: 'Structures & Unions', body: '<h2>Complex Data Structures in C</h2><p>Structures and unions allow you to create complex data types and organize related data efficiently.</p><h3>Advanced Structure Usage</h3><pre><code>// Structure definition<br>typedef struct {<br>    char name[50];<br>    int id;<br>    float salary;<br>} Employee;<br><br>// Linked list node<br>typedef struct Node {<br>    int data;<br>    struct Node* next;<br>} Node;<br><br>// Function to create new node<br>Node* createNode(int data) {<br>    Node* newNode = malloc(sizeof(Node));<br>    if (newNode) {<br>        newNode->data = data;<br>        newNode->next = NULL;<br>    }<br>    return newNode;<br>}</code></pre>' },
  { id: 'c6', title: 'Dynamic Memory Management', body: '<h2>Advanced Pointer Operations and Memory Management</h2><p>Dynamic memory management is crucial for creating flexible and efficient C programs.</p><h3>Memory Allocation</h3><pre><code>// Dynamic array allocation<br>int* createDynamicArray(int size) {<br>    int *array = (int*)calloc(size, sizeof(int));<br>    if (!array) {<br>        fprintf(stderr, "Allocation failed\\n");<br>        return NULL;<br>    }<br>    return array;<br>}<br><br>// Memory pool example<br>typedef struct {<br>    char memory[1024];<br>    int used[32];<br>} MemoryPool;<br><br>void* poolAlloc(MemoryPool* pool, size_t size) {<br>    // Find free block<br>    for (int i = 0; i < 32; i++) {<br>        if (!pool->used[i]) {<br>            pool->used[i] = 1;<br>            return &pool->memory[i * 32];<br>        }<br>    }<br>    return NULL;<br>}</code></pre>' },
  { id: 'c7', title: 'File I/O & System Programming', body: '<h2>Advanced File Operations and System Interface</h2><p>File handling and system programming are essential for creating practical C applications.</p><h3>File Operations</h3><pre><code>// Binary file operations<br>typedef struct {<br>    int id;<br>    char name[50];<br>    double salary;<br>} Record;<br><br>void writeBinaryFile(const char* filename, Record records[], int count) {<br>    FILE* file = fopen(filename, "wb");<br>    if (!file) {<br>        perror("Error opening file");<br>        return;<br>    }<br>    fwrite(records, sizeof(Record), count, file);<br>    fclose(file);<br>}<br><br>// Directory operations<br>#include &lt;dirent.h&gt;<br>void listDirectory(const char* path) {<br>    DIR* dir = opendir(path);<br>    if (!dir) return;<br>    <br>    struct dirent* entry;<br>    while ((entry = readdir(dir)) != NULL) {<br>        printf("%s\\n", entry->d_name);<br>    }<br>    closedir(dir);<br>}</code></pre>' }
]);

// Enhanced Python course with comprehensive lessons  
add('python', [
  { id: 'p4', title: 'Advanced Functions & Decorators', body: '<h2>Advanced Function Concepts in Python</h2><p>Python\'s flexible function system supports advanced programming patterns and functional programming concepts.</p><h3>Function Parameters</h3><pre><code># Various parameter types<br>def comprehensive_function(<br>    required_arg,<br>    default_arg="default",<br>    *args,<br>    keyword_only,<br>    **kwargs<br>):<br>    print(f"Required: {required_arg}")<br>    print(f"Args: {args}")<br>    print(f"Kwargs: {kwargs}")<br><br># Decorators<br>def timer(func):<br>    import time<br>    def wrapper(*args, **kwargs):<br>        start = time.time()<br>        result = func(*args, **kwargs)<br>        end = time.time()<br>        print(f"{func.__name__} took {end - start:.4f} seconds")<br>        return result<br>    return wrapper<br><br>@timer<br>def slow_function():<br>    time.sleep(1)<br>    return "Done"</code></pre>' },
  { id: 'p5', title: 'Object-Oriented Programming Mastery', body: '<h2>Advanced Object-Oriented Programming in Python</h2><p>Python\'s OOP features support sophisticated design patterns and programming paradigms.</p><h3>Advanced Class Features</h3><pre><code># Comprehensive class example<br>class BankAccount:<br>    bank_name = "Python Bank"<br>    _account_counter = 0<br>    <br>    def __init__(self, owner, initial_balance=0):<br>        self.owner = owner<br>        self._balance = initial_balance<br>        self.__account_id = self._generate_account_id()<br>        BankAccount._account_counter += 1<br>    <br>    @classmethod<br>    def _generate_account_id(cls):<br>        return f"ACC{cls._account_counter + 1:06d}"<br>    <br>    @property<br>    def balance(self):<br>        return self._balance<br>    <br>    def deposit(self, amount):<br>        if amount > 0:<br>            self._balance += amount<br>            return self._balance<br>        raise ValueError("Amount must be positive")</code></pre>' },
  { id: 'p6', title: 'Concurrency & Async Programming', body: '<h2>Concurrent and Asynchronous Programming</h2><p>Modern Python applications often need to handle multiple tasks concurrently for better performance.</p><h3>Threading and Async</h3><pre><code>import threading<br>import asyncio<br>from concurrent.futures import ThreadPoolExecutor<br><br># Threading example<br>class ThreadSafeCounter:<br>    def __init__(self):<br>        self._value = 0<br>        self._lock = threading.Lock()<br>    <br>    def increment(self):<br>        with self._lock:<br>            self._value += 1<br><br># Async function<br>async def async_hello(name, delay):<br>    print(f"Hello {name}, starting...")<br>    await asyncio.sleep(delay)<br>    print(f"Hello {name}, finished!")<br>    return f"Processed {name}"<br><br># Run async tasks<br>async def main():<br>    tasks = [<br>        async_hello("Alice", 1.0),<br>        async_hello("Bob", 1.5)<br>    ]<br>    results = await asyncio.gather(*tasks)<br>    print(results)</code></pre>' }
]);
